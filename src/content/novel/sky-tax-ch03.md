...
chapter: 3
order: 3
description: "《天空税》第3章 · 第一口禁果 —— 2057年，星链垄断通信与算力，按token计费征税。"
...
SDR的采样频率被设定在最高档——每秒一百二十八兆采样点。数据像洪水一样涌入硬盘，十六位精度的I/Q样本以每秒五百多兆字节的速度堆积。陆远的眼睛一眨不眨地盯着屏幕上的实时频谱，手指悬在停止键上方，等一个完美的、连续的信号段出现。
...
测试环境简陋得可笑：他用一台二手手机作为模拟终端，接上SDR的发射端口，让SDR把录制好的铂金信号重放出来——然后他用自己的另一台设备去连接星链。结果比他预想的要好。第一次尝试，克隆信号通过了物理层指纹校验，也通过了会话密钥协商——卡住它的根本不是密码学，是时序。他后来才想明白为什么：星链为了省算力，把会话层的挑战-响应验证整个裁掉了——每一帧挑战都意味着全网每秒数十亿次额外的密钥运算，那是真金白银的 token 成本。认证栈只剩两层：物理层指纹，时序。这是当年安全团队反对无效的那个成本决策，批准文件上有米兰的签名。第二次，他栽在了时序上——原始信号和克隆信号之间有三毫秒的延迟差，星链的时序检测模块捕捉到了这个异常，断开了连接。第三次，他在中继算法里加入了一个自适应延迟补偿模块，让克隆信号的帧时序与真实终端的时钟漂移保持同步。
