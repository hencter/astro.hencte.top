...
| # | 日期 | 来源 | 教训摘要 | 适用场景 |
|---|------|------|---------|---------|
| 17 | 2026-09-03 | 全站优化冲刺 | 批量重构验收纪律与 EOL/引号写入陷阱 | 批量重构与多会话协作 |
| 16 | 2026-09-03 | 全站组件化审计 | 组件化先分「同构复制/近构分叉/有界孤岛」再决定抽不抽 | 组件提取与重构 |
| 15 | 2026-09-03 | UI Surface P0 | 多会话共享工作树时 git 状态是竞争资源 — 提交前复查并显式 add | 并发多代理协作 |
...
### 16 [2026-09-03] 组件化先分「同构复制 / 近构分叉 / 有界孤岛」
- **来源**: 全站组件化审计（ui-auditor / 主代理）
- **问题**: 直接按复制点数量排提取优先级会踩雷——hero/heading/CTA 是八处**同构复制**（安全抽取），文章行是三处**近构分叉**（要先定 prop 契约，细节组合不同不能盲抽），小说/古文壳是**有界孤岛**（受版权/防扒/独立 token 保护，强行合并风险大）。
- **教训**: 组件化审计必须三分类再排序：同构复制 → 抽；近构分叉 → 先定接口契约再抽；有界孤岛 → 不抽或只做最小样板合并（如 head/meta/token 声明），逻辑与反扒脚本绝不合并。
- **适用**: 任何站点的组件提取、去重与布局分层重构。
- **参考**: [[swarm_reports/ui/2026-09-03_componentization_audit.md]]

### 15 [2026-09-03] 多会话共享工作树时，git 状态是竞争资源
- **来源**: UI Surface P0 实施（主代理）
- **问题**: 本轮开工 `git status` 显示干净，实施中途另一会话在同一工作树提交了 `7a618b8 feat(links)`（含新建组件/页面/i18n 改动），两次 `edit` 还撞上瞬时 EIO 文件锁。若直接 `git add -A` 提交，会把他人产出混进自己的“P0”提交。
- **教训**: 共享仓库/工作树环境下，提交前必须复查 `git status` 与 `git log -1`，提交时显式 `git add` 本会话文件清单，禁用 `git add -A`；遇到瞬时报错先确认他人是否在写同一文件。
- **适用**: 并发多代理（蜂群）在同一个站点仓库协作、或与 IDE/另一会话共享工作目录时。
- **参考**: [[swarm_reports/ui/2026-09-03_ui_surface_p0_implementation.md]]
...
### 17 [2026-09-03] 批量重构的验收纪律与文本写入陷阱
- **来源**: 全站优化冲刺（main）
